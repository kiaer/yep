...
The location of the frontal zone along southeast coast of South New Zealand is determined by extracting sea surface temperature data points creating a transect perpendicular to the coast. The transect crosses the area where the penguins forage to and is illustrated by figure \ref{fig:range}. In total 21 points are extracted from each year going from $\ang{170.5}E$ and $\ang{46}S$ to $\ang{171.5}E$ and $\ang{46.5}S$. 

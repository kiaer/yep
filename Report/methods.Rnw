...
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{area1.jpg}
\caption{Region of interest. The region contains both the penguin habitat and Otago Harbour. Region of interest is made from Google Earth and shows polygon of the area from $\ang{44}S$ to $\ang{48}S$ and $\ang{165}E$ to $\ang{175}$E.}
\label{fig:ROI}
\end{figure}
...
The location of the frontal zone along southeast coast of South New Zealand is determined by extracting sea surface temperature data points creating a transect perpendicular to the coast. The transect crosses the area where the penguins forage to and is illustrated by figure \ref{fig:range}. In total 21 points are extracted from each year going from $\ang{170.5}E$ and $\ang{46}S$ to $\ang{171.5}E$ and $\ang{46.5}S$. Each data point is a mean of SSTs from March to June.  

\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{range.png}
\caption{Region of interest including red crosses showing the created transect approximately perpendicular to the coast of New Zealand.}
\label{fig:range}
\end{figure}

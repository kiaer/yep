...
One of the aims of this projects is to see whether there is a relationship between chlorophyll concentrations and the Yellow-eyed penguin die offs. Thus a map showing the Chl concentrations in the region of interest was made (figure \ref{fig:chldot}). The concentrations clearly do not show the same tendencies as the SSTs, though there are some areas, that over the timeperiod fro 1998 to 2015 show higher concentrations than others. One such area is up coast from the Otago Peninsula, where the concentrations on average during the the New Zealand spring and summer seasons reach app. 1.6 mg/$m^3$. At the Boulder Beach Complex (red dot), the concentrations are lower, only reaching concentrations between 0.8 and 1.2 mg/$m^3$. Further away from coast, the concentrations decrease, though not in the same paste as the SST from figure \ref{fig:SST}.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.7]{clustchl.png}

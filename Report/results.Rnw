...
In the aim to state whether the measured data form Otago Harbour is representative or not, monthly anomalies of the SSTs from March-June have been calculated. Figure \ref{fig:SSTanom} shows the March-June anomalies calculated in 1985. From the figure it is seen that the temperatures in 1985 was higher than the overall mean for all years.

\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{Anom.png}
\caption{High resolution 0.05$^\circ\times 0.05^\circ$ gridpoint March-June anomalies in 1985 in the region of interest.}
\label{fig:SSTanom}
\end{figure}

Using the high resolution satellite data, a correlation map between the measured SSTs inside Otago Harbour and SSTs from satellite data was created (see figure \ref{fig:highres}). The plot shows tendencies of very high correlations, around 0.8, close to the coastline and on the westcoast of New Zealand. A correlation above 0.32 corresponds to a $p < 0.05$ indicating highly significant correlation. When reaching the Southland Current, the correlation drops dramatically. This corresponds to the SST plot in figure \ref{fig:ROI}, since when the SSTs are colder in the SAW, they then differ more from the measured temperatures within Otago Harbour hence a lower correlation.   
...
Figure \ref{fig:habitatH} shows a close-up of the area, where the population of the YEP live and forage. In this plot the correlations is more clear, showing a tend where the correlation decrease with increasing distance from coast. The plot only shows the the part of the ocean with a correlation that is significant meaning a correlation with $p > 0.05$.

\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{Boulder.png}
\caption{High resolution plot of the Boulder Beach habitat correlation with the Otago Harbour. Correlation below 0.32 is removed due to having a $p > 0.05$, resulting in only highly significant correlation being shown.}
\label{fig:habitatH}
\end{figure}
...
Using satellite data in the area shown in Figure \ref{fig:chldot}, a monthly average ranging from October to May was computed. The output contained the averages for every season ranging from 1998/1999 to 2015/2016, computed using daily cholorphyll averages during the time period.
...
Using the monthly averages, each season was analysed using a hierarchical cluster analysis. The resulting hierarchy grouped every season that shows similar trends (See Figure \ref{fig:chlclust}). As chlorophyll data is limited, only a few of the die off events can analysed with the chlorophyll data.  Looking at Figure \ref{fig:chlclust} two clusters appear, showing around 50\% similarity. The two clusters split at around levels 70\% similarities, and end up in multiple clusters showing almost completely similar seasons. The two biggest die off events since 1998, namely 2000/2001 and 2011/2012, are seen in two different groups of the cluster, therefore showing dissimilarities of above 50\%.

...
\newpage
\section*{Summary}
The Yellow-eyed penguin is found only in certain areas of New Zealand and is categorized as an endangered species on the IUCN red list. \cite{mattern} results showed that sea surface temperatures alone explained 33.2\% of the variation in fledgling survival and 36.8\% of the variation in adult survival using a Bayesian mark-recapture model.  The climate data used in \cite{mattern} is taken from inside the Otago Harbour whereas the habitat of the yellow-eyed penguin is out on the coast. A description of the biological baseline of the yellow-eyed penguin, including important physical processes that can affect biology, is used as a background for carrying out data analysis on satellite data investigating whether or not the sea surface temperatures used in \cite{mattern} is an appropriate proxy and if other physical processes can be linked to the variation in survival of the population.  
A correlation on about \textcolor{red}{0.8} between the sea surface temperature anomalies inside the Otago Harbour and the area where the yellow-eyed penguin inhabits indicated that the sea surface temperatures used in \cite{mattern} is an appropriate proxy. Besides the analyses on sea surface temperatures, analysis chlorophyll concentrations and the positioning of the Subtropical Front have been carried out. For chlorophyll concentrations no results showed interesting pattern that could explain the variation in the survival of the population. The positioning of the frontal zones showed interesting results when comparing the position with the foraging patterns of the penguins studied by \cite{mattern2013}. The position of the Subtropical front could not immediately explain any of the die-event that have happened in 1989, 2001 or 2012 but it might be of interest further investigate whether or not the position of frontal zone could explain the foraging patterns and the breeding success within the population. 
\newpage
\tableofcontents
\newpage

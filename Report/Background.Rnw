...
\textcolor{red}{Map of area - bare lige indtil vi får et andet kort}. 

\begin{figure} [ht]
\begin{center}
\includegraphics[width=10cm]{Boulder.PNG}
\end{center}
\caption{Map showing the three different locations, where the YEP is colonised and the Bould Beach Complex}
\end{figure}
...
\subsection{Effects from physical changes on biological processes}

- how is the food chain influenced by physics?

YEP feed primarily on sprat, silversides, opalfish, blue cod, arrow squid and ahuru, and according to \cite{iucn}, they are very sensitive to changes in the benthic habitat, since they depend on this for food. At the same time, predation on the YEP is also a threat, since more predators have been introduced on the mainland. Since the penguins, as mentioned in section \ref{species}, nest in forrest, such predators can have a large influence on the breeding of the Yellow-eyed penguin. Food availability and amount of predators can vary due to e.g. changes in physical parameters. Besides that, forrest fires, diseases and human influences are also thought to have a large influence on the YEP population \citep{iucn}.

Other threats, that could be interesting to look into is e.g. toxins. Toxins are poisonous subtances, that are produced in living organisms/cells. Usually the amount of toxic algal blooms increases with temperature increase (climate change). 

- how does increased temperatures etc. effect toxins?

- Another thing that can change, are the ocean currents. If they move closer to or away from the coast, it is possible, that this can have an influence on the penguins.
...
Notes for the background:
- Other parameters that could affect the penguins' survival (anthropogenic)
